    ...
    for (int round = 0; round < KECCAK_ROUNDS; round++) {
        ...
        // FIPS PUB 202 - SHA-3 Standard, 3.2.1 Specification of theta
        {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                sheet_xor[i] = state[i] ^ state[i + 5] ^ state[i + 10] ^ state[i + 15] ^ state[i + 20];
            }
        ...
        // FIPS PUB 202 - SHA-3 Standard, 3.2.4 Specification of chi
        {
            ...
            for (int x = 0; x < 5; x++) {
                for (int y = 0; y < 5; y++) {
                    ...
                    new_state[dest_index] = state[dest_index] ^ (
...
IPP_OWN_DEFN (void, sha3_hashInit, (void* pHash)) {
    PadBlock(0, pHash, IPP_SHA3_STATE_BYTESIZE);
}
