...
IPPFUN(Ipp64u, ippcpGetEnabledCpuFeatures, (void)) { return cpFeaturesMask; }
...
static int cpGetFeatures(Ipp64u* pFeaturesMask)
{
    ...
    mask = 0;
    if (edx_ & BIT23)
        mask |= ippCPUID_MMX;  // edx[23] - MMX(TM) Technology
    if (edx_ & BIT25)
        mask |= ippCPUID_SSE;  // edx[25] - Intel® Streaming SIMD Extensions (Intel® SSE)
    if (edx_ & BIT26)
        mask |= ippCPUID_SSE2; // edx[26] - Intel® Streaming SIMD Extensions 2 (Intel® SSE2)
    if (ecx_ & BIT00)
        // ecx[0]  - Intel® Streaming SIMD Extensions 3 (Intel® SSE3) (formerly codenamed Prescott)
        mask |= ippCPUID_SSE3;
    if (ecx_ & BIT09)
        // ecx[9]  - Supplemental Streaming SIMD Extensions 3 (SSSE3) (formerly codenamed Merom)
        mask |= ippCPUID_SSSE3;
    if (ecx_ & BIT22)
        mask |= ippCPUID_MOVBE; // ecx[22] - Intel® instruction MOVBE (Intel Atom® processor)
    if (ecx_ & BIT19)
        // ecx[19] - Intel® Streaming SIMD Extensions 4.1 (Intel® SSE4.1) (formerly codenamed Penryn)
        mask |= ippCPUID_SSE41;
    if (ecx_ & BIT20)
        // ecx[20] - Intel® Streaming SIMD Extensions 4.2 (Intel® SSE4.2) (formerly codenamed Nenalem)
        mask |= ippCPUID_SSE42;
    if (ecx_ & BIT28)
        // ecx[28] - Intel® Advanced Vector Extensions (Intel® AVX) (formerly codenamed Sandy Bridge)
        mask |= ippCPUID_AVX;
    if ((ecx_ & 0x18000000) == 0x18000000) {
        tmp = (Ipp32u)cp_is_avx_extension();
        if (tmp & BIT00)
            mask |= ippAVX_ENABLEDBYOS; // Intel® AVX is supported by OS
    }
    if (ecx_ & BIT25)
        mask |= ippCPUID_AES;           // ecx[25] - Intel® AES New Instructions
    if (ecx_ & BIT01)
        mask |= ippCPUID_CLMUL;         // ecx[1]  - Intel® instruction PCLMULQDQ
    if (ecx_ & BIT30)
        mask |= ippCPUID_RDRAND;        // ecx[30] - Intel® instruction RDRRAND
    if (ecx_ & BIT29)
        mask |= ippCPUID_F16C;          // ecx[29] - Intel® instruction F16C
    // Intel® AVX2 instructions extension: only if 3 features are enabled at once:
    // FMA, Intel® AVX 256 int & GPR BMI (bit-manipulation);
    if (ecx_ & BIT12)
        flgFMA = 1;
    else
        flgFMA = 0;       // ecx[12] - FMA 128 & 256 bit
    if (idBaseMax >= 7) { // get CPUID.eax = 7
        ...
        if (ebx_ & BIT05)
            ...
            flgINT = 0; //ebx[5], Intel® Advanced Vector Extensions 2 (Intel® AVX2) (int 256bits)
        // ebx[3] - enabled ANDN, BEXTR, BLSI, BLSMK, BLSR, TZCNT
        // ebx[8] - enabled BZHI, MULX, PDEP, PEXT, RORX, SARX, SHLX, SHRX
        if ((ebx_ & BIT03) && (ebx_ & BIT08))
            flgGPR = 1;
        else
            flgGPR = 0; // VEX-encoded GPR instructions (GPR BMI)
        // Intel® architecture formerly codenamed Broadwell instructions extension
        if (ebx_ & BIT19)
            // eax[0x7] -->> ebx:: Bit 19: Intel® instructions ADOX/ADCX
            mask |= ippCPUID_ADCOX;
        if (ebx_ & BIT18)
            // eax[0x7] -->> ebx:: Bit 18: Intel® instruction RDSEED
            mask |= ippCPUID_RDSEED;
        if (ebx_ & BIT29)
            // eax[0x7] -->> ebx:: Bit 29: Intel® Secure Hash Algorithm Extensions
            mask |= ippCPUID_SHA;

        if (ecx_ & BIT09)
            mask |= ippCPUID_AVX2VAES;   // ecx[09] - Intel® Vector AES instruction set
        if (ecx_ & BIT10)
            mask |= ippCPUID_AVX2VCLMUL; // ecx[10] - Intel® instruction VPCLMULQDQ

        // Intel® Advanced Vector Extensions 512 (Intel® AVX-512) extension
        if (ebx_ & BIT16)
            mask |= ippCPUID_AVX512F;  // ebx[16] - Intel® AVX-512 Foundation
        if (ebx_ & BIT26)
            mask |= ippCPUID_AVX512PF; // ebx[26] - Intel® AVX-512 Pre Fetch Instructions (PFI)
        if (ebx_ & BIT27)
            // ebx[27] - Intel® AVX-512 Exponential and Reciprocal Instructions (ERI)
            mask |= ippCPUID_AVX512ER;
        if (ebx_ & BIT28)
            mask |= ippCPUID_AVX512CD; // ebx[28] - Intel® AVX-512 Conflict Detection
        if (ebx_ & BIT17)
            mask |= ippCPUID_AVX512DQ; // ebx[17] - Intel® AVX-512 Dword & Quadword
        if (ebx_ & BIT30)
            mask |= ippCPUID_AVX512BW; // ebx[30] - Intel® AVX-512 Byte & Word
        if (ebx_ & BIT31)
            mask |= ippCPUID_AVX512VL; // ebx[31] - Intel® AVX-512 Vector Length Extensions (VLE)
        if (ecx_ & BIT01)
            // ecx[01] - Intel® AVX-512 Vector Bit Manipulation Instructions
            mask |= ippCPUID_AVX512VBMI;
        if (ecx_ & BIT06)
            // ecx[06] - Intel® AVX-512 Vector Bit Manipulation Instructions 2
            mask |= ippCPUID_AVX512VBMI2;
        if (edx_ & BIT02)
            // edx[02] - Intel® AVX-512 Vector instructions for deep learning enhanced word variable precision
            mask |= ippCPUID_AVX512_4VNNIW;
        if (edx_ & BIT03)
            // edx[03] - Intel® AVX-512 Vector instructions for deep learning floating-point single precision
            mask |= ippCPUID_AVX512_4FMADDPS;
        // bitwise OR between ippCPUID_MPX & ippCPUID_AVX flags can be used to define that arch is GE than formerly codenamed Skylake
        if (ebx_ & BIT14)
            mask |= ippCPUID_MPX; // ebx[14] - Intel® Memory Protection Extensions (Intel® MPX)
        if (ebx_ & BIT21)
            mask |= ippCPUID_AVX512IFMA;   // ebx[21] - Intel® AVX-512 IFMA PMADD52

        if (ecx_ & BIT08)
            mask |= ippCPUID_AVX512GFNI;   // test bit ecx[08]
        if (ecx_ & BIT09)
            mask |= ippCPUID_AVX512VAES;   // test bit ecx[09]
        if (ecx_ & BIT10)
            mask |= ippCPUID_AVX512VCLMUL; // test bit ecx[10]

        cpGetReg((int*)buf, 0x7, 1);       // Intel® SM3, SM4, and SHA512 instructions set check
        eax_ = (Ipp32u)buf[0];
        if (eax_ & BIT00)
            mask |= ippCPUID_AVX2SHA512;   // test bit eax[00]
        if (eax_ & BIT01)
            mask |= ippCPUID_AVX2SM3;      // test bit eax[01]
        if (eax_ & BIT02)
            mask |= ippCPUID_AVX2SM4;      // test bit eax[02]
    ...
    if (idExtdMax >= 0x80000001) {              // get CPUID.eax=0x80000001
        ...
        if (ecx_ & BIT08)
            mask |=
                ippCPUID_PREFETCHW; // eax[0x80000001] -->> ecx:: Bit 8: Intel® instruction PREFETCHW
    }
...
IPPFUN(int, ippcpGetEnabledNumThreads, (void)) { return cpthreads_omp_of_n_ipp; }


#define AVX3X_FEATURES                                                              \
    (ippCPUID_AVX512F | ippCPUID_AVX512CD | ippCPUID_AVX512VL | ippCPUID_AVX512BW | \
     ippCPUID_AVX512DQ)
#define AVX3M_FEATURES \
...
IppStatus owncpFeaturesToIdx(Ipp64u* cpuFeatures, int* index)
{
   ...
   */
    ...
    if ((mask != (*cpuFeatures & mask)) && (ownStatus == ippStsNoErr))
        ownStatus =
            ippStsFeaturesCombination; /* warning if combination of features is incomplete */
    *cpuFeatures |= mask;
    return ownStatus;
}
