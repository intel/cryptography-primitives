...
IPPFUN(IppStatus, ippsAES_GCMProcessIV, (const Ipp8u* pIV, int ivLen, IppsAES_GCMState* pState))
{
    ...
    * mode, p.5.2.1.1 Input Data) the IV shall be between 1 and 2^64 bits. */
    ...
    IPP_BADARG_RET(!(GcmInit == AESGCM_STATE(pState) || GcmIVprocessing == AESGCM_STATE(pState)),
                   ippStsBadArgErr);
    ...
    if (AESGCM_BUFLEN(pState)) {
        ...
        if (BLOCK_SIZE == AESGCM_BUFLEN(pState)) {
            ivHashUpdate(&AES_GCM_KEY_DATA(pState),
                         &AES_GCM_CONTEXT_DATA(pState),
    ...
    if (AESGCM_BUFLEN(pState)) {
        ...
        XorBlock(pIV,
                 AESGCM_COUNTER(pState) + AESGCM_BUFLEN(pState),
                 AESGCM_COUNTER(pState) + AESGCM_BUFLEN(pState),
                 locLen);
