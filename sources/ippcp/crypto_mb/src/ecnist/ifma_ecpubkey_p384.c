...
#if (_MBX>=_MBX_K1)
   status |= internal_avx512_nistp384_ecpublic_key_ssl_mb8(pa_pubx, pa_puby, pa_pubz, pa_skey, pBuffer, use_jproj_coords);
#else
...
#if (_MBX>=_MBX_K1)
   status |= internal_avx512_nistp384_ecpublic_key_mb8(pa_pubx, pa_puby, pa_pubz, pa_skey, pBuffer, use_jproj_coords);
#else
   status = MBX_SET_STS_ALL(MBX_STATUS_UNSUPPORTED_ISA_ERR);
#endif /* #if (_MBX>=_MBX_K1) */
   return status;
