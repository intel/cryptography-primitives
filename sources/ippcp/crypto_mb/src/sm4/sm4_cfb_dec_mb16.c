...
DLL_PUBLIC
mbx_status16 OWNAPI(mbx_sm4_decrypt_cfb128_mb16)(int8u* pa_out[SM4_LINES], const int8u* pa_inp[SM4_LINES], const int len[SM4_LINES], const mbx_sm4_key_schedule* key_sched, const int8u* pa_iv[SM4_LINES])
{
    int buf_no;
    ...
    if (MBX_IS_ANY_OK_STS16(status))
        status |= sm4_cfb128_dec_kernel_mb16(pa_out, (const int8u**)pa_inp, (const int*)len, (const int32u**)key_sched, pa_iv, (__mmask16)mb_mask);
#else
