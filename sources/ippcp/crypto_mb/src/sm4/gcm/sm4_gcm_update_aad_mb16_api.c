...
DLL_PUBLIC
mbx_status16 OWNAPI(mbx_sm4_gcm_update_aad_mb16)(const int8u *const pa_aad[SM4_LINES], const int aad_len[SM4_LINES], SM4_GCM_CTX_mb16 *p_context)
{
   int buf_no;
   ...
   if (MBX_IS_ANY_OK_STS16(status)) {
      __mmask16 overflow_mask = sm4_gcm_update_aad_mb16(pa_aad, aad_len, (__mmask16)mb_mask, p_context);
