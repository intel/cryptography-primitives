...
#define USE_AMS
#ifdef USE_AMS
#define SQUARE_52x20_mb8(out, Y, mod, k0) AMS52x20_diagonal_mb8((int64u *)out, (int64u *)Y, (int64u *)mod, (int64u *)k0);
#else
#define SQUARE_52x20_mb8(out, Y, mod, k0) ifma_amm52x20_mb8((int64u *)out, (int64u *)Y, (int64u *)Y, (int64u *)mod, (int64u *)k0);
#endif
                  ...
                  int64u work_buffer[][8])
{
   ...
   */
   ...
   for (int idx = 1; idx < (1 << EXP_WIN_SIZE) / 2; idx++) {
      ...
      ifma_ahmm52x20_mb8((int64u *)red_table[2 * idx + 1],
   ...
   for (exp_bit_no -= EXP_WIN_SIZE; exp_bit_no >= 0; exp_bit_no -= EXP_WIN_SIZE) {
      ...
      AMS4x52x20_diagonal_stitched_with_extract_mb8((int64u *)red_Y,
                                                    (U64 *)mulb,
                                                    (U64 *)mulbx,
                                                    (int64u *)red_Y,
                                                    (int64u *)modulus,
...
#define USE_AMS
#ifdef USE_AMS
#define SQUARE_52x20_mb4(out, Y, mod, k0) AMS52x20_diagonal_mb4((int64u *)out, (int64u *)Y, (int64u *)mod, (int64u *)k0);
#else
#define SQUARE_52x20_mb4(out, Y, mod, k0) ifma_amm52x20_mb4((int64u *)out, (int64u *)Y, (int64u *)Y, (int64u *)mod, (int64u *)k0);
#endif
...
static void extract_multiplier_mb4(int64u out[LEN52][4], int64u tbl[][LEN52][4], const __m256i idx_target)
{
   extract_multiplier_mb4_N((__m256i *)out, (const __m256i *)tbl, idx_target, LEN52, EXP_WIN_SIZE);
}

void EXP52x20_mb4(int64u out[][4],
                  const int64u base[][4],
                  const int64u exp[][4],
...
{
   /* other window sizes are not supported at the moment */
   assert(EXP_WIN_SIZE == 4);
    ...
    * redx_table[i] = (base^i)*(2^((LEN52/2)*DIGIT_SIZE)) mod modulus, for i=0, 1, ... , (2^EXP_WIN_SIZE)-1
    */
   arr_pint64u_x4 red_table  = (arr_pint64u_x4)(expz + LEN64 + 1); // total of LEN52<<EXP_WIN_SIZE = LEN52 * 2^EXP_WIN_SIZE arr_pint64u_x4 structures
   arr_pint64u_x4 redx_table = (arr_pint64u_x4)(expz + LEN64 + 1 + (LEN52 << EXP_WIN_SIZE));

   __ALIGN64 int64u_x4 mulb[LEN52];
   __ALIGN64 int64u_x4 mulbx[LEN52];

   // compute table of powers base^i, i=0, ..., (2^EXP_WIN_SIZE) -1
   _mm256_storeu_si256((__m256i *)red_X, _mm256_set1_epi64x(1));
   zero_mb4(&red_X[1], LEN52 - 1); /* table[0] = mont(x^0) = mont(1) */

   // red_table[0] = R = 2^(LEN52*DIGIT_SIZE) mod modulus
   ifma_amm52x20_mb4((int64u *)red_table[0], (int64u *)red_X, (int64u *)toMont, (int64u *)modulus, (int64u *)k0);
   // redx_table[0] = R*2^(-(LEN52/2)*DIGIT_SIZE) mod modulus = 2^((LEN52/2)*DIGIT_SIZE) mod modulus
   ifma_ahmr52x20_mb4((int64u *)redx_table[0], (int64u *)red_table[0], (int64u *)modulus, (int64u *)k0);

   // red_table[1] = base*2^(LEN52*DIGIT_SIZE) mod modulus
   ifma_amm52x20_mb4((int64u *)red_table[1], (int64u *)base, (int64u *)toMont, (int64u *)modulus, (int64u *)k0);
   // redx_table[1] = base*2^((LEN52/2)*DIGIT_SIZE) mod modulus
   ifma_ahmr52x20_mb4((int64u *)redx_table[1], (int64u *)red_table[1], (int64u *)modulus, (int64u *)k0);

   for (int idx = 1; idx < (1 << EXP_WIN_SIZE) / 2; idx++) {
      // red_table[2*idx] = (base^(2*idx))*2^(LEN52*DIGIT_SIZE) mod modulus
      SQUARE_52x20_mb4((int64u *)red_table[2 * idx], (int64u *)red_table[idx], (int64u *)modulus, (int64u *)k0);

      // redx_table[2*idx] = (base^(2*idx))*2^((LEN52/2)*DIGIT_SIZE) mod modulus
      ifma_ahmr52x20_mb4((int64u *)redx_table[2 * idx], (int64u *)red_table[2 * idx], (int64u *)modulus, (int64u *)k0);

      // red_table[2*idx+1] = (base^(2*idx+1))*2^(LEN52*DIGIT_SIZE) mod modulus
      ifma_ahmm52x20_mb4((int64u *)red_table[2 * idx + 1],
                         (int64u *)red_table[2 * idx],
                         (int64u *)red_table[1],
                         (int64u *)redx_table[1],
                         (int64u *)modulus,
                         (int64u *)k0);
      // redx_table[2*idx+1] = (base^(2*idx+1))*2^((LEN52/2)*DIGIT_SIZE) mod modulus
      ifma_ahmr52x20_mb4((int64u *)redx_table[2 * idx + 1], (int64u *)red_table[2 * idx + 1], (int64u *)modulus, (int64u *)k0);
   }
   ...
   /* process 1-st exp window - just init result */
   __m256i red_table_idx0 = ((const __m256i *)expz[exp_bit_no / 64])[0];

   red_table_idx0 = _mm256_srli_epi64(red_table_idx0, exp_bit_no % 64);

   extract_multiplier_mb4(red_Y, red_table, red_table_idx0);
   ...
   for (exp_bit_no -= EXP_WIN_SIZE; exp_bit_no >= 0; exp_bit_no -= EXP_WIN_SIZE) {
      ...
      red_table_idx0 = ((const __m256i *)expz[exp_chunk_no])[0];
      red_table_idx0 = _mm256_srl_epi64(red_table_idx0, shift_right_n);

      const __m256i T0 = _mm256_sll_epi64(((const __m256i *)expz[exp_chunk_no + 1])[0], shift_left_n);

      red_table_idx0 = _mm256_and_si256(_mm256_xor_si256(red_table_idx0, T0), table_idx_mask);

      /* 4 squarings stitched with extracting pre-computed multiplier from the table */
      AMS4x52x20_diagonal_stitched_with_extract_mb4(
         (int64u *)red_Y, (U64 *)mulb, (U64 *)mulbx, (int64u *)red_Y, (int64u *)modulus, (int64u *)k0, red_table, redx_table, red_table_idx0);

      ifma_ahmm52x20_mb4((int64u *)red_Y, (int64u *)red_Y, (int64u *)mulb, (int64u *)mulbx, (int64u *)modulus, (int64u *)k0);
   }
   ...
   ifma_amm52x20_mb4((int64u *)out, (int64u *)red_Y, (int64u *)red_X, (int64u *)modulus, (int64u *)k0);
}
