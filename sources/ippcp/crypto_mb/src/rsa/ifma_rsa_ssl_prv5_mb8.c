                                       ...
                                       int expected_rsa_bitsize)
{
   ...
   /* test input pointers */
   if (NULL == from_pa || NULL == to_pa || NULL == p_pa || NULL == q_pa || NULL == dp_pa || NULL == dq_pa || NULL == iq_pa) {
      status = MBX_SET_STS_ALL(MBX_STATUS_NULL_PARAM_ERR);
      return status;
   }
   /* test rsa modulus size */
   if (RSA_1K != expected_rsa_bitsize && RSA_2K != expected_rsa_bitsize && RSA_3K != expected_rsa_bitsize && RSA_4K != expected_rsa_bitsize) {
   ...
   if (MBX_IS_ANY_OK_STS(status)) {
      ...
      case RSA_1K:
         ifma_ssl_rsa1K_prv5_layer_mb8(from_pa, to_pa, p_pa, q_pa, dp_pa, dq_pa, iq_pa);
         break;
      case RSA_2K:
         ifma_ssl_rsa2K_prv5_layer_mb8(from_pa, to_pa, p_pa, q_pa, dp_pa, dq_pa, iq_pa);
         break;
      case RSA_3K:
         ifma_ssl_rsa3K_prv5_layer_mb8(from_pa, to_pa, p_pa, q_pa, dp_pa, dq_pa, iq_pa);
         break;
      case RSA_4K:
         ifma_ssl_rsa4K_prv5_layer_mb8(from_pa, to_pa, p_pa, q_pa, dp_pa, dq_pa, iq_pa);
         break;
      }
   }
   ...
   if (MBX_IS_ANY_OK_STS(status)) {
      ...
      case RSA_1K:
         if (MBX_IS_ANY_0_TO_3_OK_STS(status))
            ifma_ssl_rsa1K_prv5_layer_mb4(&from_pa[0], &to_pa[0], &p_pa[0], &q_pa[0], &dp_pa[0], &dq_pa[0], &iq_pa[0]);
         if (MBX_IS_ANY_4_TO_7_OK_STS(status))
            ifma_ssl_rsa1K_prv5_layer_mb4(&from_pa[4], &to_pa[4], &p_pa[4], &q_pa[4], &dp_pa[4], &dq_pa[4], &iq_pa[4]);
         break;
      case RSA_2K:
         if (MBX_IS_ANY_0_TO_3_OK_STS(status))
            ifma_ssl_rsa2K_prv5_layer_mb4(&from_pa[0], &to_pa[0], &p_pa[0], &q_pa[0], &dp_pa[0], &dq_pa[0], &iq_pa[0]);
         if (MBX_IS_ANY_4_TO_7_OK_STS(status))
            ifma_ssl_rsa2K_prv5_layer_mb4(&from_pa[4], &to_pa[4], &p_pa[4], &q_pa[4], &dp_pa[4], &dq_pa[4], &iq_pa[4]);
         break;
      case RSA_3K:
         if (MBX_IS_ANY_0_TO_3_OK_STS(status))
            ifma_ssl_rsa3K_prv5_layer_mb4(&from_pa[0], &to_pa[0], &p_pa[0], &q_pa[0], &dp_pa[0], &dq_pa[0], &iq_pa[0]);
         if (MBX_IS_ANY_4_TO_7_OK_STS(status))
            ifma_ssl_rsa3K_prv5_layer_mb4(&from_pa[4], &to_pa[4], &p_pa[4], &q_pa[4], &dp_pa[4], &dq_pa[4], &iq_pa[4]);
         break;
      case RSA_4K:
         if (MBX_IS_ANY_0_TO_3_OK_STS(status))
            ifma_ssl_rsa4K_prv5_layer_mb4(&from_pa[0], &to_pa[0], &p_pa[0], &q_pa[0], &dp_pa[0], &dq_pa[0], &iq_pa[0]);
         if (MBX_IS_ANY_4_TO_7_OK_STS(status))
            ifma_ssl_rsa4K_prv5_layer_mb4(&from_pa[4], &to_pa[4], &p_pa[4], &q_pa[4], &dp_pa[4], &dq_pa[4], &iq_pa[4]);
         break;
      }
   }
#else
