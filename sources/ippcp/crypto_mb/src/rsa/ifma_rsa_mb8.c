                                      ...
                                      int8u *pBuffer)
{
   ...
   */
   if (MBX_IS_ANY_OK_STS(status)) {
      ...
      ifma_cp_rsa_pub_layer_mb4(&from_pa[0], &to_pa[0], &n_pa[0], expected_rsa_bitsize, meth, buffer);
      ifma_cp_rsa_pub_layer_mb4(&from_pa[4], &to_pa[4], &n_pa[4], expected_rsa_bitsize, meth, buffer);
#else
...
#elif ((_MBX >= _MBX_L9) && _MBX_AVX_IFMA_SUPPORTED)
      ifma_cp_rsa_prv2_layer_mb4(&from_pa[0], &to_pa[0], &d_pa[0], &n_pa[0], expected_rsa_bitsize, meth, buffer);
      ifma_cp_rsa_prv2_layer_mb4(&from_pa[4], &to_pa[4], &d_pa[4], &n_pa[4], expected_rsa_bitsize, meth, buffer);
#else
      status = MBX_SET_STS_ALL(MBX_STATUS_UNSUPPORTED_ISA_ERR);
#endif /* #if (_MBX >= _MBX_K1) */
#if !defined(NO_USE_MALLOC)
...
#if (_MBX >= _MBX_K1)
      ifma_cp_rsa_prv5_layer_mb8(from_pa, to_pa, p_pa, q_pa, dp_pa, dq_pa, iq_pa, expected_rsa_bitsize, meth, buffer);
#elif ((_MBX >= _MBX_L9) && _MBX_AVX_IFMA_SUPPORTED)
      ifma_cp_rsa_prv5_layer_mb4(&from_pa[0], &to_pa[0], &p_pa[0], &q_pa[0], &dp_pa[0], &dq_pa[0], &iq_pa[0], expected_rsa_bitsize, meth, buffer);
      ifma_cp_rsa_prv5_layer_mb4(&from_pa[4], &to_pa[4], &p_pa[4], &q_pa[4], &dp_pa[4], &dq_pa[4], &iq_pa[4], expected_rsa_bitsize, meth, buffer);
#else
      status = MBX_SET_STS_ALL(MBX_STATUS_UNSUPPORTED_ISA_ERR);
#endif /* #if (_MBX >= _MBX_K1) */
#if !defined(NO_USE_MALLOC)
