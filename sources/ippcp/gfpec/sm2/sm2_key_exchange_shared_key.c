...
/* clang-format on */
{
    ...
    */
    ...
    if (IsFeatureEnabled(ippCPUID_AVX512IFMA) && (cpID_PrimeTPM_SM2 == ECP_MODULUS_ID(pEC))) {
        const IppStatus sts = gfec_key_exchange_sm2_shared_key_avx512(pSharedKey,
                                                                      sharedKeySize,
                                                                      pSSelf,
                                                                      pPrvKey,
                                                                      pEphPrvKey,
...
#endif // (_IPP32E >= _IPP32E_K1)
    {
        ...
        const IppsGFpECPoint* pEphPubKey     = (ippKESM2Requester == role)
        ...
        BNU_CHUNK_T* pDataEphPrv = BN_NUMBER(pEphPrvKey); // Ephemeral Private Key -> r(a/b)
                                                          ...
                                                          // r(a/b)*G
        gfec_MulBasePoint(&pTmpPoint, pDataEphPrv, BN_SIZE(pEphPrvKey), pEC, pScratchBuffer);
       ...
       */
        ...
        /* (d(a/b) + x(a/b)`*r(a/b) ) mod n */
        cpModAdd_BNU(t, pDataEphPrv, pTmpPrv, pOrder, ordLen, pBuffTmp);

        /* 5) U/V = [h*t(a/b)]( P(b/a) + [x(b/a)`]R(b/a) ) = ( x(u/v), y(u/v) ) */
        /* [x(b/a)`]R(b/a) */
        const int scalarBits = ((elemBits + 1) / 2);
        cpEcGFpInitPoint(&pTmpPoint, cpEcGFpGetPool(1, pEC), 0, pEC);
        gfec_point_mul(ECP_POINT_X(&pTmpPoint),
                       ECP_POINT_X(pEphPubKey),
                       (Ipp8u*)_xs_,
                       scalarBits,
                       pEC,
                       pScratchBuffer);
