...
IPPCP_INLINE void cpMaskedReplace_ct(BNU_CHUNK_T* dst,
                                     const BNU_CHUNK_T* src,
                                     int len,
                                     BNU_CHUNK_T replaceMask)
{
    int n;
    for (n = 0; n < len; n++)
...
*/
IPPCP_INLINE void cpMaskedCopyBNU_ct(BNU_CHUNK_T* dst,
                                     BNU_CHUNK_T mask,
                                     const BNU_CHUNK_T* src1,
                                     const BNU_CHUNK_T* src2,
                                     int len)
{
    int i;
    for (i = 0; i < len; i++)
...
/* tests if MSB(a)==1 */
IPPCP_INLINE BNU_CHUNK_T cpIsMsb_ct(BNU_CHUNK_T a)
{
    return (BNU_CHUNK_T)0 - (a >> (sizeof(a) * 8 - 1));
}

/* tests if LSB(a)==1 */
IPPCP_INLINE BNU_CHUNK_T cpIsLsb_ct(BNU_CHUNK_T a) { return (BNU_CHUNK_T)0 - (a & 1); }

/* tests if a is odd */
IPPCP_INLINE BNU_CHUNK_T cpIsOdd_ct(BNU_CHUNK_T a) { return cpIsLsb_ct(a); }

/* tests if a is even */
IPPCP_INLINE BNU_CHUNK_T cpIsEven_ct(BNU_CHUNK_T a) { return ~cpIsLsb_ct(a); }

/* tests if a==0 */
IPPCP_INLINE BNU_CHUNK_T cpIsZero_ct(BNU_CHUNK_T a) { return cpIsMsb_ct(~a & (a - 1)); }

/* tests if a==b */
IPPCP_INLINE BNU_CHUNK_T cpIsEqu_ct(BNU_CHUNK_T a, BNU_CHUNK_T b) { return cpIsZero_ct(a ^ b); }

/* test if a<b */
IPPCP_INLINE BNU_CHUNK_T cpIsLt_ct(BNU_CHUNK_T a, BNU_CHUNK_T b)
{
...
IPPCP_INLINE BNU_CHUNK_T cpIsEquBlock_ct(const void* pSrc1, const void* pSrc2, int len)
{
...
/* r = mask? a : b */
IPPCP_INLINE BNU_CHUNK_T cpSelect_ct(BNU_CHUNK_T mask, BNU_CHUNK_T a, BNU_CHUNK_T b)
{
    return (mask | b) & (~mask | a);
}

IPPCP_INLINE int cpSelect_ct_int(BNU_CHUNK_T mask, int a, int b)
{
    return (int)cpSelect_ct(mask, (BNU_CHUNK_T)a, (BNU_CHUNK_T)b);
}

IPPCP_INLINE Ipp8u cpSelect_ct_8u(BNU_CHUNK_T mask, Ipp8u a, Ipp8u b)
{
