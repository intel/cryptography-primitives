...
__IPPCP_INLINE __m512i getInputBlocks(__m128i* const currentState,
...
/* clang-format on */
{
    ...
    if (blocks) {
        ...
        for (int i = 0; i < blocks; i++) {
            ...
            switch (i) {
            case 0:
                c = _mm512_extracti64x2_epi64(ciphLsb0, 0);
                break;
            case 1:
                c = _mm512_extracti64x2_epi64(ciphLsb0, 1);
                break;
            default:
                c = _mm512_extracti64x2_epi64(ciphLsb0, 2);
            }
            currentState = _mm_mask_compress_epi8(c, blocksCompressMask, currentState);
            inpBlk0      = _mm512_mask_broadcast_i64x2(inpBlk0,
                                                  (__mmask8)(0x03 << ((i + 1) << 1)),
                                                  currentState);
        }
