...
/* Intel® Advanced Vector Extensions 512 (Intel® AVX512) -
   Intel® Xeon® Phi(TM) Processor (formerly Knights Landing)*/
#define _IPP_I0 512

/* Intel® Advanced Vector Extensions 512 (Intel® AVX512) -
   Intel® Xeon® Processor (formerly codenamed Skylake)*/
#define _IPP_S0 1024
...
#define IPPFUN(type, name, arg) \
    __attribute__((force_align_arg_pointer)) extern type IPP_CALL name arg
#else
#define IPPFUN(type, name, arg) extern type IPP_CALL name arg
#endif
#endif
...
#define IPP_ALIGNED_PTR(ptr, align) \
    (void*)((unsigned char*)(ptr) + (IPP_BYTES_TO_ALIGN(ptr, align)))
...
#define IPP_CONTEXT(a, b, c, d) \
    (int)(((unsigned)(a) << 24) | ((unsigned)(b) << 16) | ((unsigned)(c) << 8) | (unsigned)(d))
...
#define IPP_BADARG_RET(expr, ErrCode) \
    {                                 \
        if (expr) {                   \
            IPP_ERROR_RET(ErrCode);   \
        }                             \
    }

#define IPP_BAD_SIZE_RET(n) IPP_BADARG_RET((n) <= 0, ippStsSizeErr)

#define IPP_BAD_STEP_RET(n) IPP_BADARG_RET((n) <= 0, ippStsStepErr)

#define IPP_BAD_PTR1_RET(ptr) IPP_BADARG_RET(NULL == (ptr), ippStsNullPtrErr)

#define IPP_BAD_PTR2_RET(ptr1, ptr2) \
    {                                \
        IPP_BAD_PTR1_RET(ptr1);      \
        IPP_BAD_PTR1_RET(ptr2)       \
    }

#define IPP_BAD_PTR3_RET(ptr1, ptr2, ptr3) \
    {                                      \
        IPP_BAD_PTR2_RET(ptr1, ptr2);      \
        IPP_BAD_PTR1_RET(ptr3)             \
    }

#define IPP_BAD_PTR4_RET(ptr1, ptr2, ptr3, ptr4) \
    {                                            \
        IPP_BAD_PTR2_RET(ptr1, ptr2);            \
        IPP_BAD_PTR2_RET(ptr3, ptr4)             \
    }
